...
mesh_lr, buffer = [buildMesh2d(points, max_edge=0.4, min_edge=0.1, lowrank=r, density_neighbors=8) for r in  lr]

print(f"low-rank mesh: {len(mesh_lr[0].points)}, {len(mesh_lr[1].points)} vertices "
      f"(target {round(lr[0] * len(points))}, {round(lr[1] * len(points))})")
...
axs[1].triplot(
    mesh_lr[0].points[:, 0], mesh_lr[0].points[:, 1],
    mesh_lr[0].cells_dict["triangle"], linewidth=0.5,
)
axs[1].set_title(f"Low-rank mesh, lowrank={lr[0]} ({len(mesh_lr[0].points)} vertices)")

axs[2].triplot(
    mesh_lr[1].points[:, 0], mesh_lr[1].points[:, 1],
    mesh_lr[1].cells_dict["triangle"], linewidth=0.5,
)
axs[2].set_title(f"Low-rank mesh, lowrank={lr[1]} ({len(mesh_lr[1].points)} vertices)")
